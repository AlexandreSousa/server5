...
if ((isset($_GET['id_cliente'])) && ($_GET['id_cliente'] != "")) {
  $deleteSQL = sprintf("DELETE FROM logados WHERE id_cliente=%s",
...
shell_exec("/usr/bin/sudo /sbin/iptables -t nat -D PREROUTING -s $ip -p tcp --dport 80 -j REDIRECT --to-port $squid 2>&1 1> /dev/null");
 ...
 echo "<meta http-equiv='refresh' content='0;URL=?pg=logados'>"; 
}
...
mysql_select_db($database_Conexao, $Conexao);
$query_qr_desloga = sprintf("SELECT * FROM logados WHERE id_cliente = %s", GetSQLValueString($colname_qr_desloga, "int"));
